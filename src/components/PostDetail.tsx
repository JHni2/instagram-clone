    ...
    <section className="flex w-full h-full">
      ...
      <div className="w-full basis-2/5 flex flex-col">
        ...
        <ul className="border-t border-gray-200 h-full overflow-y-auto p-4 mb-1">
          {comments &&
            comments.map(({ image, username: commnetUsername, comment }, index) => (
              <li className="flex items-center mb-1" key={index}>
                <Avatar image={image} size="small" highlight={commnetUsername === username} />
                <div className="ml-2">
        ...
        <ActionBar onComment={postComment} post={post} />
      </div>
    </section>
  );
}
