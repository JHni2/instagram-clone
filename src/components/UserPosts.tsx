...
const tabs = [
  { type: 'posts', icon: <PostIcon className="w-3 h-3" />, title: 'User posts' },
    ...
    <section>
      <ul className="flex justify-center uppercase">
        {tabs.map(({ type, icon, title }) => (
          <li className={`mx-12 p-4 cursor-pointer text-neutral-500 border-black ${type === query && '!text-black font-bold border-t'}`} key={type} onClick={() => setQuery(type)}>
